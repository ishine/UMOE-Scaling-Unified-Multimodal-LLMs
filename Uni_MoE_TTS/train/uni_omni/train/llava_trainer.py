...
class LLaVATrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if self.args.tune_speech_generator:
            embed = get_mm_adapter_state_maybe_zero_3(self.model.named_parameters(), ["speech_gen"])
        ...
        embed_lm = get_mm_adapter_state_maybe_zero_3(self.model.named_parameters(), ["model.embed_tokens",".lm_head"])
        embed_lm = {(k[11:] if k.startswith('base_model.') else k): v for k, v in embed_lm.items()}
        if any(k.startswith('model.model.') for k in embed_lm):
            embed_lm = {(k[6:] if k.startswith('model.') else k): v for k, v in embed_lm.items()}
        if self.args.local_rank == 0 or self.args.local_rank == -1:
            self.model.config.save_pretrained(output_dir)
            torch.save(embed_lm, os.path.join(output_dir, f'embed_lm.bin'))
