...
# from uni_moe.model.speech_generator_AR.builder import build_ar_speech_generator
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        if config.whisper_tower:
            self.audio_tower = build_whisper_tower(config, delay_load=True)###  need change delay_load
        ...
        self.padding_side = "left"  # set it to left by default, user can use setter to change padding_side
        ...
        if hasattr(model_args, "audio_tower") and model_args.audio_tower is not None:
            ...
            if pretrain_audio_aligner is not None:
                ...
                for k,v in self.audio_aligner.named_parameters():
                    if "query_tokens" in k:
                        print("after:",k,v)
                # self.whisper_query_tokens = nn.Parameter([v for k, v in whisper_aligner_weights.items()][0])
    ...
    def initialize_image_generator(self, model_args):
        ...
        if self.tune_image_generator and getattr(self, "image_generator", None) is None:
        ...
        if image_grid_thw is not None or video_grid_thw is not None:
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    # )
                    range_tensor = range_tensor.float() * second_per_grid_t
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums + audio_nums):
                    ...
                    if not audio_mode:
                        ...
                        tokens_per_second = 3.0 # following qwen2.5 vl setting
                        range_tensor = range_tensor.float() * tokens_per_second
                        range_tensor = torch.round(range_tensor).to(base_position_ids.device).to(base_position_ids.dtype)
    ...
    def forward(
        ...
        # print(f"input len {input_ids.size()}")
        # print("ids:",input_ids)
        # print("audios:",audio_features)
        ...
        if inputs_embeds is None:
            # print("ori_ids",input_ids)
            inputs_embeds = self.model.embed_tokens(input_ids)
            # print("ori",inputs_embeds)
            if pixel_values is not None:
                ...
                print("image_embeds",image_embeds.shape) #这里需要统一是[576, 3584]，但是会遇到[864, 3584]
                
                # if golden_caption_embedding is not None:
            ...
            # whisper feartures:[batch_size&chunk_num, 30, 8000]
            if audio_features is not None:
        ...
        # print("shapes after",inputs_embeds.shape,labels.shape)
        # print("audio",inputs_embeds)
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
        ...
        if labels is not None:
            ...
            ### image gen loss
            # print(f"golden_caption_embedding: {golden_caption_embedding}")
            # print(f"golden_task_embedding: {golden_task_embedding}")
            # print(f"golden_visual_embedding: {golden_visual_embedding}")
            ...
            if golden_caption_embedding is not None:
                ...
                img_start_pos = (labels == img_token_ids[0]).nonzero(as_tuple=False)[:, 1].tolist()
                img_end_pos = (labels == img_token_ids[-1]).nonzero(as_tuple=False)[:, 1].tolist()
                ...
                golden_task_emb = torch.load(golden_task_embedding[0], map_location="cuda").unsqueeze(0).to(device)
                golden_caption_emb = torch.load(golden_caption_embedding[0], map_location="cuda").to(device)

                task_loss = self.l2_loss(task_hidden_embedding[0].to(device), golden_task_emb.to(device)).mean()
                caption_loss = self.l2_loss(caption_hidden_embedding[0].to(device), golden_caption_emb.to(device)).mean() # 后俩维为空
            
                #latent_loss, golden_visual_emb = self.image_generator.forward(img_path, tar_path, prompt_feats=caption_hidden_embedding[0].to(device), prompt_pooled_feats=task_hidden_embedding[0].to(device), input_clip_x=golden_visual_emb[:1].to(device)) # alpha4
                if pixel_values is not None:
                    #latent_loss, golden_visual_emb = self.image_generator.forward(img_path, tar_path, prompt_feats=caption_hidden_embedding[0].to(device), prompt_pooled_feats=golden_task_emb.to(device), input_clip_x=golden_visual_emb.to(device)) #finetune_imggen_s1
                    #latent_loss, golden_visual_emb = self.image_generator.forward(img_path, tar_path, prompt_feats=golden_caption_emb.to(device), prompt_pooled_feats=task_hidden_embedding[0].to(device), input_clip_x=golden_visual_emb.to(device)) #finetune_imggen_s2
                    latent_loss, golden_visual_emb = self.image_generator.forward(img_path, tar_path, prompt_feats=caption_hidden_embedding[0].to(device), prompt_pooled_feats=task_hidden_embedding[0].to(device), input_clip_x=visual_hidden_embeddings.to(device)) #finetune_imggen_s3 s4 s5  

                    ###
                    visual_loss = self.l2_loss(visual_hidden_embeddings.to(device), golden_visual_emb.to(device)).mean()
                    print(f"loss: {loss.item()}, caption_loss: {caption_loss.item()}, task_loss: {task_loss.item()}, visual_loss: {visual_loss.item()}, latent_loss: {latent_loss.item()}")
                else:
                    latent_loss, golden_visual_emb = self.image_generator.forward(img_path, tar_path, prompt_feats=caption_hidden_embedding[0].to(device), prompt_pooled_feats=task_hidden_embedding[0].to(device))  
                    visual_loss = torch.tensor(0.0, device=loss.device)
                    print(f"loss: {loss.item()}, caption_loss: {caption_loss.item()}, task_loss: {task_loss.item()}, latent_loss: {latent_loss.item()}")
                
                #loss = loss + caption_loss.to(loss.device) + latent_loss.to(loss.device)  #finetune_imggen_s1
                #loss = loss + task_loss.to(loss.device) + latent_loss.to(loss.device)  #finetune_imggen_s2
                loss = loss + caption_loss.to(loss.device) + task_loss.to(loss.device) + visual_loss.to(loss.device) + latent_loss.to(loss.device) #finetune_imggen_s3 s4 s5
